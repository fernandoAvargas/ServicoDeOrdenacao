...
{
    public class ServiceOrderByAuthorAndTitleAndEditionYear : IBooksOrderer
    {
        public List<BookModel> BooksOrderer(List<BookModel> BookList, int order)
        {
            ...
            if (order == asc)
            {
                ListBooks = (from o in BookList
                             orderby o.authorName ascending, o.title descending, o.editionYear descending
                             select o).ToList();
            }
            else if (order == desc)
            {

                ListBooks = BookList.OrderByDescending(o => o.authorName).ThenBy(o => o.title).ThenBy(o => o.editionYear).ToList();
            }
