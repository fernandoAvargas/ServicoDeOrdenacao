...
{
    public class ServiceOrderByEditionYearAndIdAndTitle : IBooksOrderer
    {
        ...
        public List<BookModel> BooksOrderer(List<BookModel> BookList, int order)
        {
            ...
            if (order == asc)
            {
                ListBooks = (from o in BookList
                             orderby o.editionYear ascending, o.idBook descending, o.title descending
                             select o).ToList();
            }
            else if (order == desc)
            {

                ListBooks = BookList.OrderByDescending(o => o.editionYear).ThenBy(o => o.idBook).ThenBy(o => o.title).ToList();
            }
