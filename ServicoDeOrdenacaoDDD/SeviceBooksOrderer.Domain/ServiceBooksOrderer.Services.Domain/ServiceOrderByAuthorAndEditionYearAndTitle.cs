...
{
    public class ServiceOrderByAuthorAndEditionYearAndTitle : IBooksOrderer
    {
        public List<BookModel> BooksOrderer(List<BookModel> BookList, int order)
        {
            ...
            if (order == asc)
            {
                ListBooks = (from o in BookList
                             orderby o.authorName ascending, o.editionYear descending, o.title descending
                             select o).ToList();
            }
            else if (order == desc)
            {

                ListBooks = BookList.OrderByDescending(o => o.authorName).ThenBy(o => o.editionYear).ThenBy(o => o.title).ToList();
            }
