        ...
        public List<BookModel> BooksOrderer(List<BookModel> BookList, int order)
        {
            ...
            if (order == asc)
            {
                ListBooks = (from o in BookList
                             orderby o.title, o.idBook, o.authorName
                             select o).ToList();
            }
            else if (order == desc)
            {
                ListBooks = BookList.OrderByDescending(o => o.title).ThenByDescending(o => o.idBook).ThenByDescending(o => o.authorName).ToList();
            }
