...
{
    public class ServiceOrderByIdAndTitleAndAuthor : IBooksOrderer
    {
        public List<BookModel> BooksOrderer(List<BookModel> BookList, int order)
        {
            ...
            if (order == asc)
            {
                ListBooks = (from o in BookList
                             orderby o.idBook ascending, o.title descending, o.authorName descending
                             select o).ToList();
            }
            else if (order == desc)
            {

                ListBooks = BookList.OrderByDescending(o => o.idBook).ThenBy(o => o.title).ThenBy(o => o.authorName).ToList();
            }
