...
{
    public class ServiceOrderByEditionYearAndAuthor : IBooksOrderer
    {
        public List<BookModel> BooksOrderer(List<BookModel> BookList, int order)
        {
            ...
            if (order == asc)
            {
                ListBooks = (from o in BookList
                             orderby o.editionYear ascending, o.authorName descending
                             select o).ToList();
            }
            else if (order == desc)
            {

                ListBooks = BookList.OrderByDescending(o => o.editionYear).ThenBy(o => o.authorName).ToList();
            }
