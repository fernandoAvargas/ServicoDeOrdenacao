...
{
    public class ServiceOrderByTitleAndEditionYearAndId : IBooksOrderer
    {
        public List<BookModel> BooksOrderer(List<BookModel> BookList, int order)
        {
            ...
            if (order == asc)
            {
                ListBooks = (from o in BookList
                             orderby o.title ascending, o.editionYear descending, o.idBook descending
                             select o).ToList();
            }
            else if (order == desc)
            {

                ListBooks = BookList.OrderByDescending(o => o.title).ThenBy(o => o.editionYear).ThenBy(o => o.idBook).ToList();
            }
