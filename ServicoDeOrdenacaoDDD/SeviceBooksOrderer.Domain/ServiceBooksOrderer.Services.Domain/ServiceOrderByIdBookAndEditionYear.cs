...
{
    public class ServiceOrderByIdBookAndEditionYear : IBooksOrderer
    {
        public List<BookModel> BooksOrderer(List<BookModel> BookList, int order)
        {
            ...
            if (order == asc)
            {
                ListBooks = (from o in BookList
                             orderby o.idBook ascending, o.editionYear descending
                             select o).ToList();
            }
            else if (order == desc)
            {

                ListBooks = BookList.OrderByDescending(o => o.idBook).ThenBy(o => o.editionYear).ToList();
            }
