        ...
        public List<BookModel> BooksOrderer(List<BookModel> BookList, int order)
        {
            ...
            if (order == asc)
            {
                ListBooks = (from o in BookList
                             orderby o.authorName, o.title, o.idBook
                             select o).ToList();
            }
            else if (order == desc)
            {

                ListBooks = BookList.OrderByDescending(o => o.authorName).ThenByDescending(o => o.title).ThenByDescending(o => o.idBook).ToList();
            }
